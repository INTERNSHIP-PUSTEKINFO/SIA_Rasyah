...
$filter_mapel_id = isset($_GET['mapel_id']) ? mysqli_real_escape_string($koneksi, $_GET['mapel_id']) : '';
...
// Query untuk mengambil data mata pelajaran, semester, dan tahun ajaran untuk dropdown filter
$query_mapel = "SELECT id, nama_mapel FROM mapel ORDER BY nama_mapel ASC";
$result_mapel = mysqli_query($koneksi, $query_mapel);

$query_semester = "SELECT DISTINCT semester FROM nilai ORDER BY semester ASC";
...
if (!empty($filter_mapel_id)) {
    $where_clauses[] = "n.mapel_id = '$filter_mapel_id'";
}
...
<div class="container mt-5">
    <h3><i class="fas fa-chart-bar me-2"></i>Data Nilai Siswa</h3>
    <div class="d-flex mb-4">
        <a href="admin_nilai_tambah.php" class="btn btn-primary me-2"><i class="fas fa-plus-circle me-1"></i> Tambah Nilai</a>
    ...
    <div class="card mb-4">
        ...
        <form method="GET" action="admin_nilai.php">
            <div class="row g-3">
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    <select name="mapel_id" class="form-select">
                        ...
                        <?php while ($mapel = mysqli_fetch_assoc($result_mapel)): ?>
                            <option value="<?= $mapel['id']; ?>" <?= ($filter_mapel_id == $mapel['id']) ? 'selected' : ''; ?>>
                ...
                <div class="col-md-2">
                    ...
                    <?php if (!empty($filter_nama) || !empty($filter_mapel_id) || !empty($filter_semester) || !empty($filter_tahun_ajaran)): ?>
                        <a href="admin_nilai.php" class="btn btn-secondary w-100 mt-2"><i class="fas fa-sync-alt me-1"></i> Reset</a>
                    <?php endif; ?>
                </div>
            </div>
        </form>
                ...
                <?php if (mysqli_num_rows($result) > 0): $no = 1; while ($row = mysqli_fetch_assoc($result)): ?>
                    <tr>
                        ...
                        <td class="text-center">
                            <a href="admin_nilai_edit.php?id=<?= htmlspecialchars($row['id']); ?>" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i> Edit</a>
                            <a href="admin_nilai_hapus.php?id=<?= htmlspecialchars($row['id']); ?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin ingin menghapus nilai ini?')"><i class="fas fa-trash-alt"></i> Hapus</a>
                        </td>
                    </tr>
                <?php endwhile; else: ?>
