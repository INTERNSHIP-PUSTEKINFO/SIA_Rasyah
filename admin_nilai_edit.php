...
// Ambil data siswa dan mapel untuk dropdown
...
if (isset($_POST['update'])) {
    ...
    $updateStmt = mysqli_prepare($koneksi, "
        UPDATE nilai SET
            siswa_nis = ?,
            mapel_id = ?,
            nilai_tugas = ?,
    ...
    mysqli_stmt_bind_param($updateStmt, "siddssssi", 
        $siswa_nis, 
        $mapel_id, 
        $nilai_tugas, 
        $nilai_uts, 
    ...
    if (mysqli_stmt_execute($updateStmt)) {
        header("Location: admin_nilai.php?msg=update_success");
        exit;
    } else {
        $pesan = "Gagal memperbarui nilai: " . mysqli_error($koneksi);
        $alert_type = 'danger';
    }
}
?>
...
<div class="container mt-5">
    <h3><i class="fas fa-edit me-2"></i> Edit Nilai Siswa</h3>
    <div class="card p-4">
        <?php if (!empty($pesan)): ?>
        ...
        <form method="post">
            <div class="mb-3">
                ...
                <select name="siswa_nis" id="siswa_nis" class="form-select" required>
                    <?php while ($s = mysqli_fetch_assoc($siswaResult)): ?>
                        <option value="<?= htmlspecialchars($s['nis']); ?>" <?= $data['siswa_nis'] == $s['nis'] ? 'selected' : ''; ?>>
            ...
            <div class="mb-3">
                ...
                <select name="mapel_id" id="mapel_id" class="form-select" required>
                    <?php while ($m = mysqli_fetch_assoc($mapelResult)): ?>
                        <option value="<?= htmlspecialchars($m['id']); ?>" <?= $data['mapel_id'] == $m['id'] ? 'selected' : ''; ?>>
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <input type="number" step="0.01" name="nilai_tugas" id="nilai_tugas" class="form-control" value="<?= htmlspecialchars($data['nilai_tugas']); ?>" required>
                </div>
                <div class="col-md-4 mb-3">
                    <label for="nilai_uts" class="form-label">Nilai UTS</label>
                    <input type="number" step="0.01" name="nilai_uts" id="nilai_uts" class="form-control" value="<?= htmlspecialchars($data['nilai_uts']); ?>" required>
                </div>
                <div class="col-md-4 mb-3">
                    <label for="nilai_uas" class="form-label">Nilai UAS</label>
                    <input type="number" step="0.01" name="nilai_uas" id="nilai_uas" class="form-control" value="<?= htmlspecialchars($data['nilai_uas']); ?>" required>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="tahun_ajaran" id="tahun_ajaran" class="form-control" value="<?= htmlspecialchars($data['tahun_ajaran']); ?>" required>
                </div>
            </div>

            <button type="submit" name="update" class="btn btn-success me-2"><i class="fas fa-save me-1"></i> Simpan Perubahan</button>
