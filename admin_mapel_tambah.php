...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($kode_mapel) || empty($nama_mapel) || empty($jurusan_id)) {
        ...
        $stmt = mysqli_prepare($koneksi, "INSERT INTO mapel (kode_mapel, nama_mapel, jurusan_id) VALUES (?, ?, ?)");
        mysqli_stmt_bind_param($stmt, "ssi", $kode_mapel, $nama_mapel, $jurusan_id);
        $sukses = mysqli_stmt_execute($stmt);

        if ($sukses) {
            header("Location: admin_mapel.php?msg=berhasil");
            exit;
        } else {
            $pesan = "Gagal menyimpan mata pelajaran: " . mysqli_error($koneksi);
            $alert_type = 'danger';
        }
    }
}
...
<div class="container mt-5">
    <h3><i class="fas fa-plus-circle me-2"></i> Tambah Mata Pelajaran Baru</h3>
    <div class="card p-4">
        <?php if (!empty($pesan)): ?>
