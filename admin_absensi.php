...
if (isset($_GET['msg'])) {
    if ($_GET['msg'] === 'berhasil') {
        $status_message = "Data absensi berhasil ditambahkan.";
        $alert_type = 'success';
    } elseif ($_GET['msg'] === 'update_success') {
        $status_message = "Data absensi berhasil diperbarui!";
        $alert_type = 'success';
    } elseif ($_GET['msg'] === 'delete_success') {
        $status_message = "Data absensi berhasil dihapus!";
        $alert_type = 'success';
...
// Ambil data absensi lengkap (JOIN absensi, siswa, user, dan mapel)
$query = "
    SELECT a.id, a.tanggal, a.keterangan, 
           u.nama AS nama_siswa, a.siswa_nis, 
           m.nama_mapel
    FROM absensi a
...
<div class="container mt-5">
    <h3><i class="fas fa-clipboard-check me-2"></i> Manajemen Data Absensi Siswa</h3>
    <div class="d-flex mb-4">
        <a href="admin_absensi_tambah.php" class="btn btn-primary me-2"><i class="fas fa-plus-circle me-1"></i> Tambah Absensi</a>
                ...
                <tbody>
                    <?php if (mysqli_num_rows($result) > 0): $no = 1; while ($row = mysqli_fetch_assoc($result)): ?>
                        <tr>
                            ...
                            <td class="text-center"><?= htmlspecialchars($row['siswa_nis']) ?></td>
                            <td><?= htmlspecialchars($row['nama_mapel']) ?></td>
                            <td class="text-center"><?= htmlspecialchars($row['keterangan']) ?></td>
                            <td class="text-center">
                                <a href="admin_absensi_edit.php?id=<?= $row['id'] ?>" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i> Edit</a>
                                <a href="admin_absensi_hapus.php?id=<?= $row['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin ingin hapus data absensi ini?')"><i class="fas fa-trash-alt"></i> Hapus</a>
                            </td>
                        </tr>
                    <?php endwhile; else: ?>
